...
# Add backend and sdk/python to sys.path
backend_path = Path("c:/Users/itach/Documents/arkan-vault/backend")
sdk_path = Path("c:/Users/itach/Documents/arkan-vault/sdk/python")
...
def main():
    print("Iniciando o upload da memória do Hermes para o servidor remoto com timeout de 30s...")
    
    # Initialize client with 30s timeout to allow time for server-side embedding generation
    client = ArkanVaultClient(timeout=30.0)
...
"""
    ...
    try:
        ...
        print(f"ID gerado no servidor: {response.get('id')}")
        print(f"Título: {response.get('title')}")
    except Exception as e:
        print(f"Erro ao enviar memória para o servidor: {e}")
        import traceback
        traceback.print_exc()
